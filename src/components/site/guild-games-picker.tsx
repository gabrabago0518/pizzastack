...
import { toggleGuildGame } from "@/app/guilds/actions";
import { cn } from "@/lib/utils";
import type { Game } from "@/lib/supabase/types";

// Same shape as GamesPicker, bound to a guild's guild_games instead of a
// player's profile_games.
export function GuildGamesPicker({
  guildId,
  allGames,
  initialSelectedIds,
}: {
  ...
  function handleToggle(gameId: string) {
    ...
    startTransition(async () => {
      const result = await toggleGuildGame(guildId, gameId, willSelect);
      if (result.error) {
        // revert on failure
        setSelectedIds((prev) => {
          const next = new Set(prev);
